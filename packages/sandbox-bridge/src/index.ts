/**
 * @dsh-runbox/sandbox-bridge
 *
 * 把逐调用沙箱模式翻译成箱的围栏配置，并把强制执行度上报给上层。
 *
 * 将来接管：`ctx.sandboxPolicy（只消费）`（官方 seam 包 `@deepseek-ai/dsh-sandbox-policy`）
 * 当前状态：**M0 占位**，实现排在 M2。
 *
 * 这里刻意不假装可用：被调用时抛 `RunboxNotImplementedError`。一个"什么都不做
 * 也不报错"的 provider，比一个报错的 provider 危险得多——上层会以为自己被隔离了。
 *
 * @module @dsh-runbox/sandbox-bridge
 */
...
/** 插件名。 */
export const name = '@dsh-runbox/sandbox-bridge'

/** 依赖的 ctx 服务：执行地基。 */
export const inject = ['runbox'] as const

/** 本包在 M0 的定位说明，供 host 启动日志与插件市场展示。 */
export const summary = '把逐调用沙箱模式翻译成箱的围栏配置，并把强制执行度上报给上层'
...
export function apply(_ctx: Context & { runbox: RunboxService }): never {
  throw new RunboxNotImplementedError('ctx.sandboxPolicy（只消费）', 'M2')
}
